...
#              7, 8  systems with the internal Red Hat Satellite using
#              FTP3 credentials.
#
...
run_curl() {
    ...
    cmd="curl -ks $API_URL -H \"Content-Type: text/xml\" -d \"<?xml version='1.0' encoding='UTF-8'?><methodCall><methodName>${command}</methodName> <params><param><value>$user</value></param> <param><value>PASSWORD</value></param> $data </params> </methodCall>\""
    logthis -s "$cmd"
...
if grep -qw $HOSTNAME <<< "$RESULT"; then
    formatted_echo "WARN" 2
    logthis -E "This system is already registered or currently un-registering."
    PROCEED="y"
    if [[ "${FTP3FORCE,,}" != "y" ]]; then
    ...
    logthis -n "* Unregistering the system... "
...
#  **FIRST DETERMINE IF THIS IS VERSION 8.  DIFFERENT SPLIT REQUIRED**
## FIRST split something like "redhat-release-8.0" into "8" and "release"
RELEASE=$(echo $VERREL | cut -f3 -d"-" | cut -b1)
if [[ $RELEASE == 8 ]] ; then
