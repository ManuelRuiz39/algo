...
if [ -z "$ARCH" ] ; then
    echo "Unknown or unsupported system arch: $FTP3ARCH"
...
if [ $(echo $VERREL | sed 's/.*release-//' | cut -b1) == 5 ]; then
    ...
    RELEASE=${VERREL:0:1}
    VERSION=${VERREL:1}
elif [ $(echo $VERREL | sed 's/.*release-//' | cut -b1) == 8 ]; then
...
LOCATION="$LABEL/$RELEASE/${RELEASE}${Version}"

# Base OS packages
echo "[ftp3]" >> $RHN_SOURCE
echo "name=FTP3 yum repository" >> $RHN_SOURCE
echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/${BASEOS}os/" >> $RHN_SOURCE
echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE

if [ $APPSTREAM -ne 0 ]; then
  #Appstream packages
  echo "[ftp3-appstream]" >> $RHN_SOURCE
  echo "name=FTP3 appstream yum repository" >> $RHN_SOURCE
  echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/appstream/os/" >> $RHN_SOURCE
  echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE
...
if [ $COMMON -ne 0 ]; then
  ...
  if [ $ARCH != 'ppc64le' ]; then
      ...
      echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/rh-common/os/" >> $RHN_SOURCE
      echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE
      echo "skip_if_unavailable=1" >> $RHN_SOURCE
  fi
fi
...
if [ $CRB -ne 0 ]; then
  #CRB
  echo "[ftp3-CRB]" >> $RHN_SOURCE
  echo "name=FTP3 CRB yum repository" >> $RHN_SOURCE
  echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/codeready-builder/os/" >> $RHN_SOURCE
  echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE
  echo "skip_if_unavailable=1" >> $RHN_SOURCE
fi
...
if [ $EXTRAS -ne 0 ]; then
  ...
  echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/extras/os/" >> $RHN_SOURCE
  echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE
  echo "skip_if_unavailable=1" >> $RHN_SOURCE
fi
...
if [ $HIGHAVAILABILITY -ne 0 ]; then
  #HighAvailability
  echo "[ftp3-HighAvailability]" >> $RHN_SOURCE
  echo "name=FTP3 HighAvailability yum repository" >> $RHN_SOURCE
  echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/highavailability/os/" >> $RHN_SOURCE
  echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE
  echo "skip_if_unavailable=1" >> $RHN_SOURCE
fi
...
if [ $OPTIONAL -ne 0 ]; then
  ...
  echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/optional/os/" >> $RHN_SOURCE
  echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE
  echo "skip_if_unavailable=1" >> $RHN_SOURCE
fi
...
if [ $RESILIENTSTORAGE -ne 0 ]; then
  ...
  echo "[ftp3-ResilientStorage]" >> $RHN_SOURCE
  echo "name=FTP3 ResilientStorage yum repository" >> $RHN_SOURCE
  echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/ResilientStorage/os/" >> $RHN_SOURCE
  echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE
  echo "skip_if_unavailable=1" >> $RHN_SOURCE
fi
...
if [ $SUPPLEMENTARY -ne 0 ]; then
  ...
  echo "baseurl=$URL/redhat/$RHELPATH/$LOCATION/$ARCH/supplementary/os/" >> $RHN_SOURCE
  echo "gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-redhat-release" >> $RHN_SOURCE
  echo "skip_if_unavailable=1" >> $RHN_SOURCE
fi
